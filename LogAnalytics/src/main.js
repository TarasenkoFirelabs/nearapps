...
async function connect(nearConfig) {
    ...
    window.contract = await new nearAPI.Contract(window.walletConnection.account(), nearConfig.contractName, {
        ...
        viewMethods: ['get_blockchain_analytics'],
        // Change methods can modify the state, but you don't receive the returned value when called
        changeMethods: ['set_blockchain_analytics'],
...
document.querySelector('.deploy_contract .btn').addEventListener('click', () => {
    let analytics = [
        document.querySelector('#app_id').value,
        document.querySelector('#action_id').value,
        document.querySelector('#user_id').value,
    ];
    contract.set_blockchain_analytics(analytics).then(updateUI);

    console.log('Get from blockchain: ' + contract.get_blockchain_analytics());
});
